...
3. **Redacts declared secret values** out of every model it writes. The models
   are uploaded as CI artifacts, and an artifact is a published file. Nothing
   asserted by `scripts/check-compose-topology.py` or
   `scripts/check-config-coverage.py` reads a secret VALUE — they read keys,
   images, ports, policies — so the redacted model and the real model are
   identical for every purpose those checkers have. After redacting it also
   greps every rendered value for each secret it substituted, so a value that
   reached the model by an unexpected path is caught rather than shipped.
...
def render(shape, man, out_dir):
    ...
    # --- redaction ----------------------------------------------------------
    secrets = {
        env[k] for k in man.get("redact_keys", [])
        if env.get(k) and len(env[k]) >= 8
    }
    redacted = redact(model, set(man.get("redact_keys", [])), secrets)
    leaked = find_leaks(redacted, secrets)
    if leaked:
        fail(
            f"shape {sid}: a value declared secret survived redaction at "
            f"{leaked}.\n"
            "  The rendered models are uploaded as CI artifacts, so this refuses\n"
            "  to write the file rather than publish the value. Add the key that\n"
            "  carries it to `redact_keys` in scripts/compose-shapes.json.",
            1,
        )
...
def main():
    ...
    with open(os.path.join(out_dir, "shapes.json"), "w", encoding="utf-8") as fh:
        json.dump(
            {"shapes": selected, "interpolation_variables": sorted(all_vars)},
            fh, indent=2,
        )
        fh.write("\n")
