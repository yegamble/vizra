#!/usr/bin/env bash
#
# Decide whether every required check succeeded on one SHA. (vizra META repo;
# the rules are the same ones vizra-search uses, adapted here so the meta repo
# does not depend on a sibling checkout that is gitignored and absent in CI.)
#
