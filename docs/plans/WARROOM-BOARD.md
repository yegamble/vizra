...
| 2a | user PR1b: hardening follow-ups from the security review — `server-only` on `lib/api/fetch.ts` + `lib/config.ts` (F4), `@sha256`-pinned base image + dependency/image scan lane (F5), reject non-local `$ref` before codegen (F6) | vizra-user | user PR1 merged | Small, CI-only plus two imports. Each with a red/green demonstration. |
