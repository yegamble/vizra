...
## Closing slice (2026-09-23) — fresh builder, after verifier FAIL at `11f8975`

Taken over from the previous builder, who used both fix rounds; the chair re-planned
rather than granting a third. Branch `fix/m0-artifact-privacy-a`, start head
`11f8975ae6af08a5a0790546efd63151f5c1d6ae` (clean, equal to origin — checked). Commits
go on top: no amend, no rebase, no force-push. The hard rule (no spec authenticates,
fills a credential or touches a signed URL) still stands.

**Inputs read:** meta `AGENTS.md`, vizra-user `AGENTS.md`, the verifier's
"Re-verification at 11f8975" (R3-FINDINGS H, J, I, K; R3-4), the chair rulings at the
top of the security PLAN-REVIEW, and — read-only — vizra-core
`origin/main:.github/pinned-steps.yml` and the "pinned" sections of its AGENTS.md.

**Preflight (2026-09-23):** node v22.14.0 (the `.nvmrc` pin, via nvm), npm 10.9.2,
Playwright 1.63.0 with `chromium-1243`, perl `JSON::PP`, unzip, zip, shellcheck, gh:
all present. A production build exists. Host load 280–360 at start (other work):
infrastructure timeouts are BLOCKED halves, not failures. **CI is BLOCKED by
billing** (GitHub Actions refuses to start jobs since ~06:21Z): push once, do not
re-run, record CI as BLOCKED with complete local evidence.

### The class (R3-FINDING H)

The lane guard identified security-relevant steps by SUBSTRING, so a step that merely
MENTIONS a script satisfied the check that the script RUNS. Answer, after vizra-core's
`pinned-steps.yml`: every step the guard relies on is BYTE-EQUAL to a committed pinned
body (`.github/e2e-pinned-steps.yml`), its keys limited to the pin's; any step that
mentions a pinned script or role token but is not the pin is refused anywhere in the
workflow; invariants the pins must satisfy are asserted on the pins file itself, so
weakening the pin is a named failure too; the environment the pinned bodies run in is
default-deny (workflow `env` empty, job `env` = the one key, no `defaults`, job keys
allowlisted, `runs-on` pinned) because `BASH_ENV`, `PATH` or `defaults.run.shell` change
what a byte-equal body does.

### Plan

1. `ci(e2e)`: pins + guard rewrite + strict redactor directories + HOME refused (R3-K)
   + require-checks cases (the verifier's eight spellings incl. `/tmp/empty`, one per
   other pinned step, the environment routes, the pins-file invariants).
2. `fix(e2e)`: capture `CI`/`PLAYWRIGHT_NO_COPY_PROMPT` at config load (main process
   AND worker), compare live values against the capture at every boundary, RESTORE on
   mismatch before the context closes, fail by name; main-process check after
   collection (reporter `onBegin`, before any worker forks); new lint rule
   `vizra/no-process-env-write` for `e2e/**` minus the harness.
3. `fix(e2e)`: R3-I encoded separators in the shared patterns + corpus.
4. docs at the strength of the controls; `demonstrate.sh` D17 (the R3-4 two-liner red
   at lint and at runtime; the combined attack end to end; a controlled mutation of the
   runtime check); evidence regenerated by `npm run e2e:demos`.

`npm run ci` at every commit boundary. Evidence appended below as it is produced.

### Closing slice — audit of every step identification in `check-e2e-lane.mjs` (at `11f8975`)

Identification = the guard deciding that a step IS the step it relies on. Each is
replaced by exact deep equality with a pin; substring/regex tests survive only in the
REFUSAL direction (they decide a step must be a pin, never that it is one).

| # | At `11f8975` (line) | Before | After |
|---|---|---|---|
| 1 | lane, :193 | `runOf(step) === "npm run e2e"`; other keys by a refusal list (`if`, `continue-on-error`, `shell`) — `working-directory`, `timeout-minutes` unread | whole step deep-equal to pin `lane` (name, env, run); exactly once |
| 2 | lane near-miss, :197 | `includes("e2e") \|\| includes("playwright")` for the message | removed; look-alikes reported by the mention scan against the closest pin |
| 3 | start step, :231-236 | `run.includes("docker run")` + `/--publish\s+(\d+):/` over ANY step | pin `start_image`; the port rule is asserted on the PINS (lane/canary URL = published port) |
| 4 | floor, :246 | `runOf(step).includes(FLOOR_COMMAND)` (so `… \|\| true` passed) | pin `floor` |
| 5 | build, :261 | `before.some(includes("docker build"))` | pin `build_image` |
| 6 | start (again), :264 | `before.some(includes("docker run"))` | pin `start_image` + order |
| 7 | fixture-free, :267 | `before.some(includes(FIXTURE_GUARD))` | pin `fixture_free` |
| 8 | canary, :277 | `runOf(step) === CANARY_COMMAND`; other keys by refusal list | pin `canary` |
| 9 | **redaction, :320** | **`runOf(step).includes("redact-artifacts.sh")`** — R3-FINDING H | pin `redact` (exact run, `id`, `if: failure()`) |
| 10 | uploaders, :348 | `UPLOADER` regex over `uses:` | exact `uses === UPLOAD_ACTION` + pin `upload`; the regex survives as a refusal-direction mention |
| 11 | canonical upload, :350 | `startsWith("actions/upload-artifact@")` | exact action@SHA inside the pin |
| 12 | upload gate, :356/:366 | `/steps\.<id>\.outcome\s*==\s*'success'/.test(if)` + `if.includes("failure()")` (so `always() \|\| (…)` passed) | `if` exact in the pin; the pins file must carry exactly `failure() && steps.redact.outcome == 'success'` |
| 13 | upload path, :402 | `uploadPath.includes("test-results")` | exact path block in the pin; allowlist asserted on the pin |
| 14 | file-wide uploader loop, :949 | `UPLOADER.test(uses)` | `uses === UPLOAD_ACTION \|\| UPLOADER_MENTION.test(uses)` (refusal direction; `ALLOWED_USES` is exact) |

Kept, all in the refusal direction: `/\b(next|npm run) dev\b/`, `includes("E2E_COVERAGE_FLOOR")`,
`includes("VIZRA_E2E_STAMP_KEY")`, the runner-command-file `includes`, the `.vizra-e2e`
path sweep, the four `UNSET_SHAPES`, `GLOB_METACHARACTERS`, and the new MENTIONS table.
Added: pins-file invariants; workflow/job key allowlists (`defaults:` refused);
`runs-on` pinned; env default-deny at workflow/job/unpinned-step scope; `HOME` by name;
upload IMMEDIATELY after redaction; redaction after lane, floor and canary.

### Closing slice — evidence so far (local; CI BLOCKED by billing)

Environment: macOS arm64, node v22.14.0, npm 10.9.2, Playwright 1.63.0, load 190–360.

| Commit | What | `npm run ci` (exit, files, tests, skipped) |
|---|---|---|
| `5225377` | pinned steps, strict redactor dirs, HOME, +34 require-checks cases | 0 — 18 / 529 / 0 |
| `74ff2f7` | capture/compare/restore, reporter check, `vizra/no-process-env-write` | 0 — 19 / 572 / 0 |
| `fbae536` | R3-I encoded separators, fragments, linear bound | 0 — 19 / 624 / 0 |
| `547ec56` | AGENTS.md, D17, ledger lines | 0 — 19 / 624 / 0 |

`require-checks_test.sh`: 216/223/0 at `5225377`; 219/226/0 at `74ff2f7`.

Red/green demonstrations recorded in the builder's private scratch (transcripts
quoted in the PR body): the eight spellings exit 0 through the `11f8975` guard and
exit 1 through the pinned guard; the two strict-directory tests fail on the
`11f8975` script and pass restored; the linearity test fails at 25 s on the old
boundary and passes restored; 39 of 41 R3-I corpus assertions fail on the `fbae536^`
programs and pass restored; the R3-3 runtime probe (variants b, c, e, f, g under
`CI=1`) writes a page snapshot with the typed value for all five on the `11f8975`
harness and none, each named, on the new one. `npm run e2e:demos` at `547ec56`: running.

### Closing slice — final state (2026-09-23)

Pushed once: `11f8975..f0f702e`, no force. Head **`f0f702eb289f801d93e161717546762b5fa8d51c`**.
Commits: `5225377` (H, K), `74ff2f7` (J), `fbae536` (I), `547ec56` (docs + D17),
`f0f702e` (demonstration transcripts).

| Check at `f0f702e` (local) | Exit | Result |
|---|---|---|
| `npm run ci` | 0 | 19 files / 624 tests / 0 skipped; hygiene 307 sources / 17 ledger lines |
| `bash scripts/ci/require-checks_test.sh` | 0 | 219 cases / 226 assertions / 0 failed |
| `npm run e2e:demos` (clean tree at `547ec56`, 08:47Z–09:54Z, load 190–390) | 0 | 148 passed / 0 blocked / 0 failed; D17 15/15; the ledger it wrote is byte-identical to the committed one |
| `CI=1 PLAYWRIGHT_NO_COPY_PROMPT=1 npx playwright test` | 0 | 18 passed, floor 9/9 9/9, 18 stamps |
| `check-coverage-floor-ran.mjs`, `harness-canary.mjs` | 0, 0 | floor + 18 stamps; 4/4 fixtures failed for their own kinds |
| lane guard, required floor, image pins, `check:contract`, eslint, shellcheck | 0 each | — |

**CI: BLOCKED (billing).** At `f0f702e` every Actions job concluded `failure` with 0
steps and the annotation "The job was not started because recent account payments have
failed or your spending limit needs to be increased"; GitGuardian (app) succeeded. Not
re-run, per the chair.

PR body rewritten to the final state (including that `11f8975` failed on H and J); the
Write tool again turned `?`/`/`/`#` into their characters in the draft —
caught and fixed before the body was published; AGENTS.md and commits unaffected
(written through Python/heredocs).

Not measured, stated as such in AGENTS.md: the `beforeAll` and page-handler rows (by
construction) and the two "detects only" rows (overridden `context`/`afterAll`; a spec
closing its own context after an error). Chair instruction respected: no new exploit
technique was researched for them.

Observation for the chair, NOT investigated (no payload built): `npx playwright test`
collects spec files in the MAIN process, and `e2e/harness/stamp.ts` keeps the per-run
stamp key in the main process's environment so workers inherit it — so the key is in
`process.env` while spec module scope runs during collection. AGENTS.md says the key
is "absent from the worker's environment"; it does not speak to the main process. This
is a stamp-forgery question outside this slice's class; worth a verifier's look.

Cleanup: image `pr8close-a7x:demonstrate` removed (the mutant image is removed by the
script); probe servers stopped; no containers left. Scratch (evidence, probes) kept in
the builder's private `closing-pr8-fzNI` directory.

**Status: READY_FOR_REVIEW.** Next: independent verifier on `f0f702e`; CI re-run by the
chair once billing is restored.

### Closing slice — fix round 1 of 2 (verifier FAIL at `f0f702e`: V-A, V-D required; V-B, V-C should; V-E, V-F nits)

Evidence read: `docs/evidence/warroom/2026-09-23-vizra-user-pr8-closing-VERIFY.md`.
One fix commit, **`4db7dce`**, then an evidence commit after `npm run e2e:demos`.

- **V-A:** policy also keyed on `GITHUB_ACTIONS === "true"` at capture. Cited, read
  2026-09-23 via the docs: Actions reference "Variables" → "Default environment
  variables": "You can't overwrite the value of the default environment variables named
  `GITHUB_*` and `RUNNER_*`. Currently you can overwrite the value of the `CI` variable.";
  "Naming conventions": "If you attempt to override the value of one of these default
  variables, the assignment is ignored."; "Workflow commands" → "Setting an environment
  variable" repeats the GITHUB_*/RUNNER_* sentence for `$GITHUB_ENV`. Stated: an
  in-process pre-load route can still delete both; layer 3 is what holds then.
  Unit tests: `V-A:` ×3 — 2 red / 1 passed before the change, 3/3 after. D16c gains
  `d16c-variable-blank-and-CI-emptied-under-GITHUB_ACTIONS-RED`.
- **V-D:** AGENTS.md (the key paragraph and forgery item 1) and `stamp.ts` narrowed to
  "absent from the WORKER"; main-process exposure stated; fix queued for PR B. No
  code change, no probe.
- **V-B / V-C / V-E:** root install lifecycle scripts refused; `record_browsers` exact
  `run:`; `with:` of checkout/setup-node and workflow `permissions:` pinned.
  require-checks 219 → 233; the 13 new refusals were `not ok` against the `f0f702e`
  guard (14 failed in that run, the 14th being the stale ledger before recomputation),
  233/240/0 after.
- **V-F:** one clause in the page-handler row.

| Check at `4db7dce` (local) | Exit | Result |
|---|---|---|
| `npm run ci` | 0 | 19 files / 627 tests / 0 skipped; hygiene 307 / 17 |
| `require-checks_test.sh` | 0 | 233 cases / 240 assertions / 0 failed |
| `CI=1 GITHUB_ACTIONS=true PLAYWRIGHT_NO_COPY_PROMPT=1 npx playwright test` | 0 | 18 passed, floor 9/9 9/9, 18 stamps |
| `check-coverage-floor-ran.mjs`, `harness-canary.mjs` (same env) | 0, 0 | — |
| shellcheck `scripts/ci/*.sh scripts/e2e/*.sh` | 0 | — |
| `npm run e2e:demos` | running | — |

Fix round 1 result: `npm run e2e:demos` at `4db7dce` (11:07Z–11:24Z) **149 passed / 0
blocked / 0 failed**, ledger byte-identical; evidence commit **`f6d245f`**, `npm run ci`
19 / 627 / 0 there, hygiene 308 / 17. V-A lane red/green: `CI= GITHUB_ACTIONS=true
PLAYWRIGHT_NO_COPY_PROMPT=` → 18 passed, exit 0 on the `f0f702e` harness; exit 1 by name
on `4db7dce`. Pushed once `f0f702e..f6d245f` (fast-forward). CI at `f6d245f`: every Actions
job `failure`, "The job was not started because recent account payments have failed…";
GitGuardian success. PR body gained a "fix round 1" section. Image removed; no
containers left. Status: READY_FOR_REVIEW. Next: verifier on `f6d245f`.

### Closing slice — fix round 2 of 2 (verifier FAIL at `f6d245f`: V-A2 required, V-D2 nit; wording only)

One commit **`cf053a3`**, sentences only, pushed as a fast-forward `f6d245f..cf053a3`.
- V-A2: the GITHUB_ACTIONS claim is scoped to a DIRECT `env:`/`$GITHUB_ENV` assignment of
  `CI` or `GITHUB_ACTIONS` itself. BASH_ENV-via-`$GITHUB_ENV`, `$GITHUB_PATH` and an
  in-process preload are named as routes that can silence layer 2, falling to layer 3,
  reasoned from GitHub's pages and the Bash manual and not built. Fixed in AGENTS.md
  (layer 2, the helper-script residual) and the ci-environment.ts header. A repo-wide grep
  found and fixed two check-e2e-lane.mjs comments plus comments in ci-environment.test.ts
  and demonstrate.sh.
- V-D2: "a test cannot pass without the harness — subject to the main-process note above,
  which is NOT CLOSED —".
- The D17d ledger lines were recomputed (ci-environment.ts is hashed).
- `npm run ci` 0 — 19 / 627 / 0, hygiene 308 / 17; `require-checks_test.sh` 0 —
  233 / 240 / 0; lane guard 0; shellcheck 0.
- `e2e:demos` not re-run (no control changed). Eight committed runtime transcripts quote
  `ci-environment.ts` code-frame line numbers, which are now 4 lines behind.
- CI at `cf053a3`: every Actions job refused (billing); GitGuardian success. Not re-run.
- Status: READY_FOR_REVIEW. This was the LAST fix round; a further FAIL makes the slice
  BLOCKED for re-planning.

### Closing slice — comment-only commit before merge (chair hold on the PASS at `cf053a3`)

**`c2ff445`**, pushed as a fast-forward `cf053a3..c2ff445`. V-A3: the `pageSnapshotProblem`
inline comment now matches the header (a DIRECT assignment cannot change
`GITHUB_ACTIONS`; a preload, a `BASH_ENV` via `$GITHUB_ENV` or a `$GITHUB_PATH` entry can
remove both, and then layer 3 holds). The replacement keeps the same line count. NIT:
`demonstrate.sh` D11 now says "a spec in a worker cannot read". The D17d ledger lines
were recomputed. Diff stat: 3 files, +9 −9; 0 non-comment code lines. `npm run ci` 0 —
19 / 627 / 0, hygiene 308 / 17; `require-checks_test.sh` 0 — 233 / 240 / 0. CI at
`c2ff445`: every Actions job refused (billing); GitGuardian success. Not re-run.
