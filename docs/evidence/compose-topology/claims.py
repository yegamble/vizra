...
def build():
    ...
    w("What the generator enforces: every line number is resolved against the live file at generation time, and it REFUSES to write anything if an anchor is missing or appears more than once — so **a line number is never quietly wrong**.")
    w("What it does NOT enforce: **the claim text, the kind and the cited cases in each row are hand-written.** Nothing checks that a row still describes its anchored sentence, or that the cases it names still turn that sentence red — read the anchored sentence rather than the row. The rule-id table further down is the exception: it is computed from `demo.sh` and each checker's `RULES`.")
    w("`claims.py --check` is run by hand as part of the local lane (recorded in `local-run.txt`). It is **not** a step of the `validate` CI lane, so CI does not notice a stale CLAIMS.md.")
    w("")
    w("")
