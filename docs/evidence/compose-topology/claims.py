...
`demo.sh` and the checkers' own `RULES` sets. The line numbers quoted in the
"Known weak spots" prose are computed too (WEAK_SPOT_LINES below), so no line
number in CLAIMS.md is typed by hand.
...
longer resolves or a weak-spot pattern matches nothing — the audit is
UNEVALUABLE and nothing is written.
"""
...
# --------------------------------------------------------------------------
# Line numbers quoted in the "Known weak spots" prose. Each list is EVERY line of
# the file matching the pattern, computed on every run: when a line moves, is
# fixed, or gains a sibling, the regenerated text differs and `--check` exits 1.
# A pattern that matches nothing is refused (exit 2) — the weak spot was fixed
# or reworded, and the bullet must be edited rather than printed with no lines.
# --------------------------------------------------------------------------

WEAK_SPOT_LINES = {
    # `printf '%s\n' "$x" | grep -q…` under `set -euo pipefail`
    "guard": ("scripts/ci-required-guard.sh",
              r"printf '%s\\n' [^|]*\| *grep -q"),
    # `printf '%s\n' "$rows" | head -n 1` under `set -uo pipefail`
    "select": ("scripts/ci-required-select.sh",
               r"printf '%s\\n' [^|]*\| *head -n"),
}


def resolve_lines(path, pattern):
    """Every line number in `path` matching `pattern`. Refuses zero matches."""
    try:
        with open(os.path.join(ROOT, path), "r", encoding="utf-8") as fh:
            lines = fh.read().splitlines()
    except OSError as err:
        fail(f"cannot read {path}: {err}")
    hits = [i + 1 for i, ln in enumerate(lines) if re.search(pattern, ln)]
    if not hits:
        fail(
            f"the weak-spot pattern {pattern!r} no longer matches any line of "
            f"{path}. The weak spot was fixed or reworded; edit its bullet in "
            f"this file rather than printing it with no line numbers."
        )
    return hits


def cite_lines(key):
    """`lines 80, 249` / `line 56` for WEAK_SPOT_LINES[key], resolved now."""
    hits = resolve_lines(*WEAK_SPOT_LINES[key])
    word = "line" if len(hits) == 1 else "lines"
    return word + " " + ", ".join(str(n) for n in hits)
...
def build():
    ...
    guard_lines = cite_lines("guard")
    select_lines = cite_lines("select")

    o = []
    w = o.append
    w("# Claims audit — VZ-ISSUE-002 compose topology")
    w("")
    w("GENERATED. Edit `docs/evidence/compose-topology/claims.py` and re-run it; do not edit this file by hand (`claims.py --check` fails on a hand edit).")
    w("What the generator enforces: every line number in this file is computed from the live file each time it runs, and none is typed by hand. The `Where` column comes from an anchor that must appear exactly once — it REFUSES to write anything if an anchor is missing or appears more than once. The line numbers under \"Known weak spots\" are every line of the named script that matches a fixed pattern — it REFUSES to write anything if a pattern matches nothing. So when a cited line moves, `claims.py --check` exits 1 until CLAIMS.md is regenerated; a committed CLAIMS.md is only as current as the last time someone ran it.")
    w("What it does NOT enforce: **the claim text, the kind, the cited cases and the \"What it does NOT guarantee\" cell in each row are hand-written**, and so is the prose of every section. Nothing checks that a row still describes its anchored sentence, that the cases it names still turn that sentence red, or that a weak-spot bullet still describes the lines it lists — read the source rather than this file. What is computed rather than written: the line numbers, the row count, and the rule-id table further down (from `demo.sh` and each checker's `RULES`).")
    w("`claims.py --check` is run by hand and its result is recorded in `local-run.txt`, but it is **not** one of the eight required local-lane commands in `docs/quality/COMMANDS.md` (§8 documents only the write form). It is **not** a step of the `validate` CI lane either, so CI does not notice a stale CLAIMS.md.")
    w("")
    w("")
    ...
    w(f"- **`scripts/ci-required-guard.sh` ({guard_lines}) and")
    w(f"  `scripts/ci-required-select.sh` ({select_lines})** still read captured")
    w("  output through a pipe under `set -o pipefail` — `printf … | grep -q` in")
    w("  the guard, `printf … | head -n 1` in the selector — the same race")
