...
req("VZ-ADMIN-003","Website settings: name, doctitle, description, timezone, privacy mode (public/private), content privacy defaults, single-profile mode",
    ...
    deps=["VZ-ADMIN-002","VZ-PRIVACY-001"], success=["Private site: anonymous → login on every route incl. object URLs, feeds, oEmbed, sitemap"], privacy=["Private site emits no public sitemap/OG"], ui=["/admin/settings/website"], evidence=EV_UI+EV_NEG, unresolved=["Q-023"])
req("VZ-BRAND-001","Logo (vector/image/text), favicon, palettes (light/dark/system + themed), default font, custom CSS with a safe boundary",
    ...
    deps=["VZ-ADMIN-002"], success=["Logo/palette/font applied; screenshots light/dark"], negative=["CSS containing url()/@import to remote hosts blocked or documented; JS injection unavailable"], ui=["/admin/settings/theme","/admin/settings/logo"], evidence=EV_UI+EV_NEG, unresolved=["Q-014"],
...
req("VZ-PAGE-001","Custom pages (terms, extra pages, links) with rich text/sanitized HTML, visibility, sort, link attributes, meta",
    ...
    deps=["VZ-ADMIN-002"], success=["Page renders; nav link"], negative=["Script tags stripped"], api=["/api/v1/admin/pages"], ui=["/admin/settings/pages","/page/{key}"], evidence=EV_API+EV_UI+EV_NEG, unresolved=["Q-014"])
...
req("VZ-I18N-001","Internationalization: default language, auto-detect, language chooser, enabled-language list, translation overrides, RTL",
    ...
    deps=["VZ-ADMIN-002"], success=["Two locales + RTL screenshot"], ui=["language chooser","/admin/settings/languages"], evidence=EV_UI, unresolved=["Q-043"])
...
req("VZ-BANNER-001","Banner/ad slots: 16 predefined placements × SFW/NSFW with sanitized HTML and 'show banners on NSFW' toggle",
    ...
    deps=["VZ-ADMIN-002"], success=["Placement renders where configured"], negative=["Script stripped/blocked by policy"], ui=["/admin/settings/banners"], evidence=EV_UI+EV_NEG, unresolved=["Q-013"])
...
req("VZ-SERVICES-001","CAPTCHA providers (reCAPTCHA v2/v3, hCaptcha, Turnstile) with threshold and forced contact-page mode",
    ...
    deps=["VZ-AUTH-001"], success=["Each provider live-tested with test keys"], negative=["Missing token rejected server-side"], ui=["/admin/settings/services"], evidence=EV_API+EV_NEG, unresolved=["Q-005"])
...
req("VZ-SERVICES-005","Analytics snippet slot (privacy-respecting) and external comment provider embed (optional)",
    ...
    deps=["VZ-ADMIN-002"], success=["Allowed provider renders; disallowed blocked"], ui=["/admin/settings/services"], evidence=EV_UI+EV_NEG, profiles=("full",), unresolved=["Q-011","Q-014"])
...
req("VZ-TENANT-001","Multi-tenancy: tenants with hostname, plans/limits, lifecycle API with HMAC-signed requests, install and password-reset endpoints, per-tenant isolation",
    ...
    deps=["VZ-FOUND-003","VZ-STORAGE-004"], success=["Two tenants on separate hostnames; cross-tenant read denied on every surface"], negative=["Unsigned tenant request rejected"], privacy=["Tenant isolation tested for search, counts, caches, federation actors"], recovery=["Backup/restore per tenant"], api=["/_/api/v4-compatible tenants + plans"], cli=["vizra tenants"], ui=["/admin/tenants"], evidence=EV_API+EV_NEG, profiles=("full",), unresolved=["Q-008"])
req("VZ-TENANT-002","Reverse-proxy dynamic config export for tenant routing",
    ...
    deps=["VZ-TENANT-001"], success=["Config renders; proxy validates"], api=["GET /_/config/caddy (loopback)"], evidence=EV_OPS, profiles=("full",), unresolved=["Q-008"])
