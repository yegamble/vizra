...
req("VZ-UPLOAD-008","Time-based expiring uploads and file delete links",
    ...
    deps=["VZ-UPLOAD-001","VZ-JOBS-001"], success=["Asset gone after expiry; delete link works once"], negative=["Delete link unguessable"], api=["expiration field; DELETE via token"], ui=["upload result panel"], evidence=EV_API+EV_UI, unresolved=["Q-015"])
...
req("VZ-UPLOAD-011","Embedded uploader plugin for third-party sites",
    ...
    deps=["VZ-UPLOAD-010"], success=["Embedded on a test page with allowed origin"], negative=["Disallowed origin refused; CSP documented"], api=["guest key"], ui=["/admin/settings/uploader-plugin"], evidence=EV_UI+EV_NEG, unresolved=["Q-029"],
...
req("VZ-MEDIA-002","Format matrix: JPEG, PNG, GIF (animated), WebP (animated), AVIF, BMP; HEIC/RAW only if approved",
    ...
    deps=["VZ-MEDIA-001","VZ-FOUND-007"], success=["Per-format fixture passes"], negative=["Unsupported format rejected with clear message"], evidence=["fixture matrix results"], unresolved=["Q-010"])
...
req("VZ-MEDIA-005","Video uploads (MP4, MOV, WebM): store, poster frame, bounded playback derivative",
    ...
    deps=["VZ-MEDIA-001"], success=["Fixture video uploads; poster decodes; plays in browser"], negative=["Oversized/long video rejected at declared limits"], evidence=EV_API+EV_UI, unresolved=["Q-009"],
