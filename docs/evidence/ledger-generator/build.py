...
out = {
  ...
  "notice": "Atomic requirement ledger produced by Prompt 00 on 2026-09-15. An entry is PLANNED/UNVERIFIED/NOT_STARTED unless a record in docs/evidence/ledger-generator/status_records.json admits IMPLEMENTED or VERIFIED. The generator refuses a status written anywhere else, refuses every other claimed status by name, checks each record's cited acceptance bullets and, for VERIFIED, that the verifier's evidence file under docs/evidence/warroom/ ENDS with a PASS on the merged head; the validate lane's scripts/check-ledger-status-remote.py proves with the GitHub API that each PR merged from that head into that merge commit, that the merge commit is on the component repo's main and, for VERIFIED, that ci-required succeeded on the verified head. A recorded entry lists its merges under `evidence`. release_status is never set by the generator. Removing or narrowing an entry requires an owner decision recorded in docs/OPEN_QUESTIONS.md. Open questions ratified 2026-09-15; DECIDED ids are listed under `decided`, remaining open ids under `unresolved` — see docs/OPEN_QUESTIONS.md ratification record.",
